...
var run = function(testcase){
  ...
  testscope.assert.content = [];
  testscope.assert.content.is = function(pred, obj, msg){
    var window = testscope.window;
    console.log(window.$(pred).text());
    console.log(obj);
    testscope.assert(is(window.$(pred).text(), obj), msg);
  }
  ...
  if ( testcase.type === "async" ) { // not async
    testcase.func.call(testscope,testscope.assert);
  } else if (testcase.type === "page") {
    var req = http.request(testcase.options, function(res) {
      ...
      res.on('end', function(){
        jsdom.env(dom.join(), [ jquery ],function(errors, window) {
          testscope.window = window;
          testcase.func.call(testscope, testscope.assert);
        });
      });
    });
...
var tester = function(desc){
  ...
  var func = function(desc, casefunc){
    ...
    var testcase = {"desc":desc, "func":casefunc,"isAsync":false, "failed":false, "type":""};
  ...
  func.async = function(desc,casefunc){
    ...
    var testcase = {"desc":desc, "func":casefunc,"isAsync":true, "failed":false, "type":"async"};
    func.cases.push(testcase);
    run(testcase);
    return func;
  }
  func.page = function(desc, options, casefunc){
    if ( !options ){
      throw "You have to provide a desc to each test case";
    } else if ( !casefunc ){
      throw "You have to provide a func to the case";
    }
    var testcase = {"desc":desc, "func":casefunc, "isAsync":true, "failed":false, "options":options, "type":"page"};
    func.cases.push(testcase);
    run(testcase);
    return func;
  }
